...
descending, filterable by `status`, `channel`, `assignedAgentId`, `attentionMode` and
`personId`) and
`GET /api/conversations/:id` including the person (name, phone), current assignment and
the channel window state (`canSendFreeform`, nullable `windowExpiresAt`) derived from the
window policy.
