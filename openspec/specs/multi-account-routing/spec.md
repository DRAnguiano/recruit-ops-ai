...
Allow several accounts per channel (multiple WhatsApp numbers, multiple Meta pages,
multiple Telegram bots) to coexist: each inbound message is routed to the account that
received it, the conversation remembers that account, and outbound replies go out through
the matching credential instead of "the active one" for the channel.
