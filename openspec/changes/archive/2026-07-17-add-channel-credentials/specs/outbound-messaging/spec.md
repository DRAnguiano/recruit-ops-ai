...
from the encrypted credential store and using the conversation's channel identity as
recipient, with API base URLs configurable via environment for testability. Channels
...
- **THEN** the sender resolves the active `telegram` bot token from the store and calls
  `sendMessage` with the `chat_id` derived from the person's channel identity, storing the
  returned message id

#### Scenario: Messenger send uses PSID from identity
- **WHEN** an agent sends text to a messenger conversation
- **THEN** the Send API is called with `recipient.id` = the person's PSID using the active
  `meta_page` credential and the returned `message_id` is stored
