...
`POST {GRAPH_API_BASE_URL}/{META_PAGE_ID}/messages` with
`{recipient: {id}, messaging_type: 'RESPONSE', message: {text}}` authenticated by
`META_PAGE_ACCESS_TOKEN`, one sender class per channel. Both env vars absent or partial →
the channel is not configured and sends respond 409 `CHANNEL_NOT_CONFIGURED`. The
response `message_id` is stored as the delivery's external message id.
...
#### Scenario: Unconfigured page env rejects send
- **WHEN** `META_PAGE_ACCESS_TOKEN` is missing
